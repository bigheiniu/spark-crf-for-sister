...
object Token {
  ...
  def deSerializer(s: String): Token = {
    //TODO: 针对 crf++ 进行了修改, 是针对分隔符
    val parts = s.split("""\|""")
    val label = parts.last
    val tags = parts.dropRight(1).dropRight(1)
...
case class Sequence (sequence: Array[Token]) extends Serializable {
  ...
  def compareSe(other: Sequence): Boolean = {
    if (this.toString == other.toString) true
    else false
  }
  ...
  def Baccompare(other: Sequence): Int = {
    val predict = this.toArray
    val test = other.toArray
    var flag = 0
    var i = 0
    var TP = 0
    while(i < test.length) {
      if (flag == 0) {
        if (test(i).label == "B-bacteria") {if (test(i).compare(predict(i)) == 1) flag = 1}
        }
      else if (flag == 1) {
         if (test(i).label == "I-bacteria") {if (test(i).compare(predict(i)) == 1) flag = 1 else flag = 0}
         else if (test(i).label == "B-bacteria") {
           TP = TP + 1
           if (test(i).compare(predict(i)) == 1) flag = 1 else flag = 0
         }
        }
        else if (test(i).label == "O"){
          flag = 0
          if (test(i).compare(predict(i)) == 1) TP = TP+1
        }
      i = i + 1
      }
    if (flag == 1) TP = TP + 1
